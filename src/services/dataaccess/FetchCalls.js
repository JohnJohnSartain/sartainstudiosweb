...
const createOptions = (httpMethodType, dataToSend, isFormData) => {
    ...
    options.body = isNormalPutOrPatchOrPost ? !isFormData ? JSON.stringify(dataToSend) : dataToSend : null;

    return options;
}

const isStandardPutOrPatchOrPost = (httpMethodType, isFormData) =>
    httpMethodType === httpVerbs.put || httpMethodType === httpVerbs.patch || (httpMethodType === httpVerbs.post && !isFormData) || httpMethodType === httpVerbs.delete;
